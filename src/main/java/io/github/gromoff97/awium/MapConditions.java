...
import io.github.gromoff97.awium.ConditionResult.Context;
...
import static io.github.gromoff97.awium.ValueMatching.containsAll;
...
public final class MapConditions {
    ...
    public static <K, V> Condition<Map<K, V>, V> valueFor(K key) {
        return ConditionFactories.condition("map contains the expected key", expectedReference(key), actual -> value(actual, key));
    }
    ...
    public static <K, V> Condition<Map<K, V>, Map.Entry<K, V>> entryFor(K key) {
        return ConditionFactories.condition("map contains the expected key", expectedReference(key), actual -> findEntry(actual, key));
    }

    public static <K, V> Condition<Map<K, V>, V> onlyValueFor(K key) {
        return ConditionFactories.condition("map contains only the expected key", expectedReference(key), actual -> {
            if (actual == null) {
                return unsatisfied("map was null");
            }
            if (actual.size() != 1) {
    ...
    private static <K, V> ConditionResult<? extends Map.Entry<K, V>> selectSingle(Map<K, V> actual,
    ...
    private static <K, V> ConditionResult<V> value(Map<K, V> actual, K key) {
        return findEntry(actual, key).mapSatisfied(Map.Entry::getValue);
    }

    private static <K, V> ConditionResult<Map.Entry<K, V>> findEntry(Map<K, V> actual, K key) {
        if (actual != null) {
        ...
        return ConditionResult.<Map.Entry<K, V>>unsatisfied(actual == null ? "map was null" : "map did not contain the expected key")
