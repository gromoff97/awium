...
final class CaughtEvaluator<S, R>
    ...
    public Evaluation<List<R>> apply(S actual) {
        int current = Math.min(results.size(), stages.size() - 1);
        ...
        if (evaluation == null) {
            return Evaluation.<List<R>>uncontrolled(new NullPointerException("condition returned null Evaluation")).withContext(context(current));
        }
        Evaluation<? extends R> contextual = evaluation.withContext(context(current));
        return results.size() == stages.size()
    ...
    private Evaluation<List<R>> capture(R result, int evaluated) {
        ...
        if (results.size() < stages.size()) {
            return Evaluation.<List<R>>unsatisfied("waiting for sequence stage " + (results.size() + 1))
                    .withContext(context(results.size(), evaluated));
        }
